...
request(url,function(err,response,body){
  if(err){
    ...
    var out = data['results'][0]['item']['brand']['name'] + ' , ' +
    '$'+data['results'][0]['item']['offers'][0]['prices']['USD'][0][1] + ' - ' +
    data['results'][0]['item']['octopart_url'];
...
/* Here we request the data from the API and post it to index*/



/*Move this to /formGet*/
...
url += "&start=0&limit=5";
  ...
  request(url,function(err,response,body){
    if(err){
      ...
      //Parse
      var data = JSON.parse(body);
      //show the first found result
      for(var i = 0; i < 5; i++)
      console.log(data['results'][i]['item']['brand']['name']);
        }
    });
    */
